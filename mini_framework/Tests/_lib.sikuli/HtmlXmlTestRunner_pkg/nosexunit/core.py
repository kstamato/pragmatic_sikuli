...
import HtmlXmlTestRunner_pkg.nosexunit.tools as ntools
...
class XTestElmt:
    ...
    def __init__(self):
        ...
        self.begin = None
        self.end = None
        self.begin_strft = None
        self.end_strft = None

    def start(self):
        '''Set the start time'''
        self.begin = time.time()
        self.begin_strft = time.strftime("%Y-%m-%d %X", time.localtime(self.begin))

    def stop(self):
        '''Set the end time'''
        self.end = time.time()
        self.end_strft = time.strftime("%Y-%m-%d %X", time.localtime(self.end))
...
class XSuite(XTestElmt):
    ...
    def writeXml(self, folder, duration):
        ...
        if self.getNbrTestsFromKinds([nconst.TEST_SUCCESS, nconst.TEST_FAIL, nconst.TEST_ERROR, ]) > 0:
            ...
            self.writeXmlOnStream(xfile, xname, duration)
            xfile.close()

    def writeXmlOnStream(self, stream, xname, duration):
        ...
        stream.write('<testsuite name="%s" tests="%d" errors="%d" failures="%d" time="%s">' % (xname, nbrTests, nbrErrors, nbrFails, duration))
...
class XTest(XTestElmt):
    ...
    def __init__(self, kind, elmt, output, err=None):
...
#            else:
            ...
            xmlOuput = filter(lambda x: x in printable, self.output)
            stream.write('<system-out>' + '<![CDATA[\n{0}]]>'.format(xmlOuput) + '</system-out>')
            tag = "success"
